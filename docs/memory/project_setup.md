...
description: Maree is a browser-only SVG pattern generator; Vite :5002 locally, syncs UI from Tessor Express API at :3001, deployed on Cloudflare Workers (huegrid) at maree.huegrid.workers.dev
type: project
---
...
- **Cloudflare Workers:** `huegrid` account (`business@huegrid.com`). Production URL: https://maree.huegrid.workers.dev *(workers.dev account subdomain renamed business-eb4 → huegrid on 2026-07-28; old URLs are dead)*. Deploys from `main` via Workers Builds (`npm run build` → `npx wrangler deploy`); config in `wrangler.jsonc` (static assets, `not_found_handling: single-page-application`). *(Migrated off Vercel 2026-06-06.)*
